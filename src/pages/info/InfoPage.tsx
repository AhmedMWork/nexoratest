    ...
    <>
      ...
      <div className="pt-24 pb-20 bg-[#0a0a0a] min-h-screen">
        <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-10">
          <SectionReveal>
            <Link to="/" className="inline-flex items-center gap-2 text-xs text-[#888] hover:text-[#ffaa33] transition-colors tracking-wider uppercase mb-8">
              <ArrowLeft className="w-3 h-3" />
              NEXORA
            </Link>
            <div className="w-14 h-14 flex items-center justify-center border border-[#ffaa33]/30 bg-[#ffaa33]/10 mb-6">
              <Icon className="w-6 h-6 text-[#ffaa33]" />
            </div>
            <p className="nexora-caption text-[#ffaa33] mb-3">Built Different</p>
            <h1 className="nexora-heading-md mb-6">{title}</h1>
            <p className="text-base sm:text-lg text-[#888] leading-relaxed max-w-3xl">{body}</p>
          </SectionReveal>
          ...
          <div className="mt-12 grid sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={item.label} className="p-5 bg-[#121212] border border-[#1e1e1e]">
                <h3 className="text-xs font-bold text-[#f3f3f3] uppercase tracking-wider mb-2">{item.label}</h3>
                <p className="text-xs text-[#666] leading-relaxed">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
