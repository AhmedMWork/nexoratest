...
## Admin / Studio access
The Studio is hidden from the public UI. Open directly:
...
Copy `.env.example` to `.env.local` locally or add the same variables in Vercel.

## Important
V3 uses `src/lib/firebase` for Firebase imports. Do not create a local folder named `src/firebase` or a `firebase/*` path alias, because that can shadow the Firebase SDK.
