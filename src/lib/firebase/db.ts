...
export async function createReview(review: Omit<Review, 'id' | 'createdAt'>): Promise<string> {
  ...
  await setDoc(newDoc, { ...review, isApproved: false, createdAt: now() });
  return newDoc.id;
}

export async function approveReview(id: string): Promise<void> { await updateDoc(doc(db, 'reviews', id), { isApproved: true }); }
