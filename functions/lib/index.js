...
exports.trackCustomerOrder = exports.updateOrderStatus = exports.createCheckoutOrder = exports.validateCoupon = void 0;
const app_1 = require("firebase-admin/app");
const firestore_1 = require("firebase-admin/firestore");
const https_1 = require("firebase-functions/v2/https");
(0, app_1.initializeApp)();
const db = (0, firestore_1.getFirestore)();
function normalizePhone(phone) { return phone.replace(/\D/g, '').replace(/^20/, '0'); }
function isValidEgyptPhone(phone) { return /^01[0125][0-9]{8}$/.test(phone); }
function generateOrderNumber() {
    const date = new Date().toISOString().slice(2, 10).replace(/-/g, '');
    const random = Math.random().toString(36).substring(2, 6).toUpperCase();
    return `NXR-${date}-${random}`;
}
function sanitizeString(value, max = 250) { return typeof value === 'string' ? value.trim().slice(0, max) : ''; }
...
function getDefaultTrackingMessage(status) {
    const messages = {
...
async function requireAdmin(uid) {
    if (!uid)
        throw new https_1.HttpsError('unauthenticated', 'Admin login is required.');
...
async function findCoupon(code) {
    ...
    if (snapshot.empty)
        return null;
    return { ref: snapshot.docs[0].ref, id: snapshot.docs[0].id, data: snapshot.docs[0].data() };
}
function calculateCouponDiscount(coupon, subtotal, products) {
    const now = new Date();
    const start = toDate(coupon.startDate);
    const end = toDate(coupon.endDate);
    if (!coupon.isActive || ['paused', 'expired', 'draft', 'archived'].includes(coupon.status || ''))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code is not active.' };
    if (start && now < start)
        return { valid: false, discount: 0, freeShipping: false, message: 'This code is not active yet.' };
    if (end && now > end)
        return { valid: false, discount: 0, freeShipping: false, message: 'This code has expired.' };
    if ((coupon.usageLimit || 0) > 0 && (coupon.usedCount || 0) >= (coupon.usageLimit || 0))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code has reached its usage limit.' };
    if (subtotal < Number(coupon.minOrderAmount || 0))
        return { valid: false, discount: 0, freeShipping: false, message: `Minimum order is ${coupon.minOrderAmount || 0} EGP.` };
    const ids = products.map((p) => p.id);
    if (coupon.allowedProductIds?.length && !ids.some((id) => coupon.allowedProductIds?.includes(id)))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code does not apply to these items.' };
    if (coupon.excludedProductIds?.length && ids.some((id) => coupon.excludedProductIds?.includes(id)))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code does not apply to one or more items.' };
    if (coupon.allowedCategories?.length && !products.some((p) => coupon.allowedCategories?.includes(String(p.category))))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code does not apply to this category.' };
    if (coupon.excludedCategories?.length && products.some((p) => coupon.excludedCategories?.includes(String(p.category))))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code does not apply to this category.' };
    if (coupon.allowedCollections?.length && !products.some((p) => coupon.allowedCollections?.includes(String(p.collection))))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code does not apply to this collection.' };
    if (coupon.excludedCollections?.length && products.some((p) => coupon.excludedCollections?.includes(String(p.collection))))
        return { valid: false, discount: 0, freeShipping: false, message: 'This code does not apply to this collection.' };
    let discount = 0;
    ...
    if (coupon.maxDiscountAmount && coupon.type === 'percentage')
        discount = Math.min(discount, Number(coupon.maxDiscountAmount));
    discount = Math.max(0, Math.min(discount, subtotal));
    return { valid: true, discount, freeShipping, message: freeShipping ? 'Free shipping applied.' : 'Code applied.' };
}
exports.validateCoupon = (0, https_1.onCall)({ region: 'europe-west1' }, async (request) => {
    const code = sanitizeString(request.data.code, 40).toUpperCase();
    const items = Array.isArray(request.data.items) ? request.data.items : [];
    const subtotal = Number(request.data.subtotal || 0);
    if (!code)
        throw new https_1.HttpsError('invalid-argument', 'Coupon code is required.');
    if (!items.length || subtotal <= 0)
        throw new https_1.HttpsError('invalid-argument', 'Cart is required.');
    const coupon = await findCoupon(code);
    if (!coupon)
        return { valid: false, discount: 0, freeShipping: false, message: 'Invalid code.' };
...
exports.createCheckoutOrder = (0, https_1.onCall)({ region: 'europe-west1' }, async (request) => {
    const incomingOrder = request.data.order;
    if (!incomingOrder)
        throw new https_1.HttpsError('invalid-argument', 'Order payload is required.');
    ...
    if (!isValidEgyptPhone(phone))
        throw new https_1.HttpsError('invalid-argument', 'A valid Egyptian phone number is required.');
    if (!governorate || !city || address.length < 5)
        throw new https_1.HttpsError('invalid-argument', 'A complete shipping address is required.');
    if (incomingOrder.paymentMethod !== 'cod')
        throw new https_1.HttpsError('invalid-argument', 'Only cash on delivery is currently available.');
    if (items.length === 0 || items.length > 30)
        throw new https_1.HttpsError('invalid-argument', 'Order items are required.');
    ...
    normalizedItems.forEach((item) => {
        if (!item.productId || !item.size || !Number.isInteger(item.quantity) || item.quantity < 1 || item.quantity > 20)
    ...
    if (!/^NXR-\d{6}-[A-Z0-9]{4}$/.test(orderNumber))
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (couponCode) {
            ...
            if (couponSnap.empty)
                throw new https_1.HttpsError('failed-precondition', 'Invalid coupon code.');
            couponRef = couponSnap.docs[0].ref;
            couponData = couponSnap.docs[0].data();
            if (couponData.firstOrderOnly) {
                ...
                if (!previousOrders.empty)
                    throw new https_1.HttpsError('failed-precondition', 'This code is valid for first orders only.');
            }
            if ((couponData.perCustomerLimit || 0) > 0) {
                ...
                if (previousCouponOrders.size >= Number(couponData.perCustomerLimit))
                    throw new https_1.HttpsError('failed-precondition', 'This code has reached your usage limit.');
            }
        }
        let subtotal = 0;
        ...
        const orderItems = productSnaps.map((snapshot, index) => {
            ...
            if (!snapshot.exists)
                throw new https_1.HttpsError('failed-precondition', 'Product not found.');
            const product = snapshot.data();
            if (!['active', undefined].includes(product.status))
                throw new https_1.HttpsError('failed-precondition', 'Product is not available.');
            const sizes = product.sizes || [];
            const sizeIndex = sizes.findIndex((entry) => entry.size.toUpperCase() === item.size);
            if (sizeIndex === -1)
                throw new https_1.HttpsError('failed-precondition', `Size ${item.size} is not available.`);
            const selectedSize = sizes[sizeIndex];
            if (selectedSize.stock < item.quantity)
                throw new https_1.HttpsError('failed-precondition', `Only ${selectedSize.stock} left in size ${item.size}.`);
            const unitPrice = Number(product.price || 0);
            if (unitPrice <= 0)
                throw new https_1.HttpsError('failed-precondition', 'Product price is not available.');
        ...
        if (couponRef)
            transaction.update(couponRef, { usedCount: firestore_1.FieldValue.increment(1), updatedAt: serverTime });
...
exports.updateOrderStatus = (0, https_1.onCall)({ region: 'europe-west1' }, async (request) => {
    const admin = await requireAdmin(request.auth?.uid);
    const orderId = sanitizeString(request.data.orderId, 120);
    const status = sanitizeString(request.data.status, 40);
    const message = sanitizeString(request.data.message, 180) || getDefaultTrackingMessage(status);
    const allowedStatuses = ['pending', 'confirmed', 'preparing', 'packed', 'shipped', 'out_for_delivery', 'delivered', 'cancelled', 'returned', 'failed'];
    if (!orderId || !allowedStatuses.includes(status))
        throw new https_1.HttpsError('invalid-argument', 'Valid order id and status are required.');
    ...
    await db.runTransaction(async (transaction) => {
        ...
        transaction.update(orderRef, {
            status,
            updatedAt: serverTime,
            trackingUpdates: firestore_1.FieldValue.arrayUnion({ status, message, timestamp: new Date(), updatedBy: admin.uid }),
        });
...
exports.trackCustomerOrder = (0, https_1.onCall)({ region: 'europe-west1' }, async (request) => {
    const orderNumber = sanitizeString(request.data.orderNumber, 30);
    const phone = normalizePhone(sanitizeString(request.data.phone, 30));
    if (!orderNumber || !isValidEgyptPhone(phone))
        throw new https_1.HttpsError('invalid-argument', 'Order number and valid phone are required.');
